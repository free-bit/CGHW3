...
void main()
{
    ...
    if(position.x==0 && position.z==0){
        ...
        neigh5.z=position.z;
        color = texture(rgbTexture, vec2((widthTexture-neigh5.x)/widthTexture, (heightTexture-neigh5.z)/heightTexture));
    ...
    //Top-left: 0,5
        ...
        // compute normal vector using also the heights of neighbor vertices
        vertexNormal=cross(neigh0-position, neigh5-position);
    }
    //Top-right: 0,1,2
        ...
        neigh2.z=position.z;
        color = texture(rgbTexture, vec2((widthTexture-neigh2.x)/widthTexture, (heightTexture-neigh2.z)/heightTexture));
    ...
    //Bottom-right: 2,3
        ...
        // compute normal vector using also the heights of neighbor vertices
        vertexNormal=cross(neigh3-position, neigh2-position);
    }
    else{
        //0th
        neigh0 = vec3(position.x, 0, position.z+1);
